...
import { YandexGroup, YandexDevice, YandexCapability, YandexModeCapabilityParameters } from '../types';
import { 
  X, 
  Wind, 
  Volume2, 
...
export const GroupFanSettingsModal: React.FC<GroupFanSettingsModalProps> = ({
  group,
  devices,
  isOpen,
  onClose,
  ...
  const oscillationCapability = firstDevice?.capabilities.find(
  ...
  const handleApply = async () => {
    ...
    // Получаем текущие значения из первого устройства в группе
    ...
    // Проверяем, включены ли устройства в группе
    const onOffCap = firstDevice?.capabilities.find(cap => cap.type === 'devices.capabilities.on_off');
    ...
    setIsLoading(true);
    try {
      await onApply(modeActions, shouldTurnOn); // Включаем устройства только если они выключены
      onClose(); // Закрываем модальное окно после успешного применения
    } catch (error) {
      console.error('Ошибка при применении настроек вентилятора группы:', error);
    } finally {
      setIsLoading(false);
