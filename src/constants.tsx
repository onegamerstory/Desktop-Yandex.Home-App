...
export const hasVideoStreamCapability = (device: YandexDevice): boolean => {
    return device.capabilities.some((cap) => cap.type === 'devices.capabilities.video_stream');
};

const PRIVACY_INSTANCE_KEYS = new Set(['camera_sw_mute']);

const getCapabilityInstance = (cap: YandexDevice['capabilities'][number]): string | undefined => {
...
export const getIconForDevice = (type: string): React.ReactNode => {
    ...
    if (t.includes('camera')) return <Video className={className} />;
